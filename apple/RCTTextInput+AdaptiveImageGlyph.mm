...
static UIImage *processGenmojiImage(NSAdaptiveImageGlyph *glyph) API_AVAILABLE(ios(18.2)) {
    ...
    UIImage *originalImage = [UIImage imageWithData:glyph.imageContent];
    if (!originalImage) {
        return nil;
    }
    
    // Resize to consistent size for better display
    ...
    // Convert to PNG to ensure proper format
...
- (void)liveMarkdown_insertAdaptiveImageGlyph:(NSAdaptiveImageGlyph *)glyph replacementRange:(NSRange)replacementRange API_AVAILABLE(ios(18.2)) {
