  ...
  function dfs(node: StackItem | string) {
    if (typeof node === 'string') {
      ...
      if (node.tag === '<>') {
        ...
        const linkString = rawLink ? unescape(rawLink[1]!) : src;

        appendSyntax('!');
        if (hasAlt) {
          appendSyntax('[');
          processChildren(unescape(alt?.[1] || ''));
          appendSyntax(']');
        }
        appendSyntax('(');
